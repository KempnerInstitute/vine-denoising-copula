#!/bin/bash
#SBATCH --job-name=vdc_paper_e9_edges
#SBATCH --output=logs/vdc_paper_e9_edges_%j.out
#SBATCH --error=logs/vdc_paper_e9_edges_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
RUN_DIR="${OUTPUT_BASE}/vdc_paper_e9_real_edge_${TS}_${SLURM_JOB_ID:-nojobid}"
...
ACNET_ROOT="${E9_ACNET_ROOT:-${REPO_ROOT}/external/ACNet}"
...
SEED_TAG="${E9_SEED:-0}"
OUT_JSON="${RUN_DIR}/results/e9_real_edge_scaling_results_seed${SEED_TAG}.json"

DATASETS="${E9_DATASETS:-gas hepmass miniboone}"

"${PYTHON_BIN}" drafts/scripts/e9_real_edge_scaling_benchmark.py \
  --device cuda \
  --acnet-device "${E9_ACNET_DEVICE:-cpu}" \
  --datasets ${DATASETS} \
  --max-train "${E9_MAX_TRAIN:-20000}" \
  --max-test "${E9_MAX_TEST:-5000}" \
  --edge-train-size "${E9_EDGE_TRAIN_SIZE:-2000}" \
  --edge-test-size "${E9_EDGE_TEST_SIZE:-2000}" \
  --max-edges-per-tree "${E9_MAX_EDGES_PER_TREE:-2}" \
  --acnet-root "${ACNET_ROOT}" \
  --acnet-epochs "${E9_EPOCHS:-300}" \
  --acnet-batch-size "${E9_BATCH_SIZE:-200}" \
  --acnet-lr "${E9_LR:-1e-3}" \
  --acnet-depth "${E9_DEPTH:-2}" \
  --acnet-widths ${E9_WIDTHS:-10 10} \
  --acnet-eval-batch-size "${E9_EVAL_BATCH_SIZE:-128}" \
  --edge-batch-size "${E9_VDC_EDGE_BATCH_SIZE:-256}" \
  --seed "${SEED_TAG}" \
  --out "${OUT_JSON}" \
  2>&1 | tee "${RUN_DIR}/logs/e9_real_edge_scaling.log"

cp "${OUT_JSON}" "drafts/paper_outputs/e9_real_edge_scaling_results_seed${SEED_TAG}.json" || true
cp "${OUT_JSON}" "drafts/paper_outputs/e9_real_edge_scaling_results.json" || true

echo ""
echo "============================================================================"
echo "DONE: E9 real-edge scaling benchmark completed at $(date)"
echo "============================================================================"
echo "Run Dir: ${RUN_DIR}"
echo "E9 JSON: ${OUT_JSON}"
echo ""
