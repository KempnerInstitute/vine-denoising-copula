...
if [ -n "${SLURM_SUBMIT_DIR:-}" ] && [ -f "${SLURM_SUBMIT_DIR}/drafts/scripts/e2_uci_benchmark.py" ]; then
...
CKPT="${E2_CHECKPOINT:-${PAPER_CHECKPOINT:-}}"
CKPT_ARGS=()
if [ -n "${CKPT}" ]; then
  CKPT_ARGS=(--checkpoint "${CKPT}")
fi
E2_EXTRA_ARGS=()
if [ "${E2_GAUSSIAN_COPULA:-1}" = "1" ]; then
  E2_EXTRA_ARGS+=(--gaussian-copula)
fi

"${PYTHON_BIN}" drafts/scripts/e2_uci_benchmark.py \
