#!/bin/bash
#SBATCH --job-name=vdc_e13_ablation_dense
#SBATCH --output=logs/vdc_e13_ablation_dense_%j.out
#SBATCH --error=logs/vdc_e13_ablation_dense_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
"${PYTHON_BIN}" drafts/scripts/e13_depth_stability_benchmark.py \
  ...
  --seed 0 \
  --dense-levels \
  --max-edges-per-tree 6 \
  --datasets gas hepmass miniboone power credit \
